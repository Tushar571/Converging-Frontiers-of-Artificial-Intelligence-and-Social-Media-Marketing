...
t1=quanteda::tokens(t1)
t1
write.csv(t1, "tokens.csv")
dtm <- DocumentTermMatrix(t1)
dtm
inspect(dtm)
...
####################################################
### Compare Perplexity with topic coherence score ####
####################################################
...
lda_models <- list(
  "2" = lda2, "3" = lda3, "4" = lda4, "5" = lda5, "6" = lda6,
  "7" = lda7, "8" = lda8, "9" = lda9, "10" = lda10, "11" = lda11,
  "12" = lda12, "13" = lda13, "14" = lda14, "15" = lda15
)

topic_numbers <- as.numeric(names(lda_models))

perplexity_scores <- sapply(lda_models, topicmodels::perplexity, newdata = d2)

calc_coherence_score <- function(model, dfm_data, top_n = 10) {
  # textmineR uses the topic-term probability matrix and document-term matrix.
  topic_term_prob <- topicmodels::posterior(model)$terms
  topic_term_prob <- topic_term_prob[, colnames(topic_term_prob) %in% featnames(dfm_data), drop = FALSE]
  
  dtm_for_coherence <- as(dfm_data[, colnames(topic_term_prob)], "dgCMatrix")
  
  topic_coherence <- textmineR::CalcProbCoherence(
    phi = topic_term_prob,
    dtm = dtm_for_coherence,
    M = top_n
  )
  
  mean(topic_coherence, na.rm = TRUE)
}

coherence_scores <- sapply(lda_models, calc_coherence_score, dfm_data = d1, top_n = 10)

score_comparison <- data.frame(
  topics = topic_numbers,
  perplexity = as.numeric(perplexity_scores),
  coherence_score = as.numeric(coherence_scores)
)

score_comparison
write.csv(score_comparison, "perplexity_coherence_comparison.csv", row.names = FALSE)

best_perplexity_k <- score_comparison$topics[which.min(score_comparison$perplexity)]
best_coherence_k <- score_comparison$topics[which.max(score_comparison$coherence_score)]

best_perplexity_k
best_coherence_k

ldatop <- data.frame(perplexity_scores)
t(ldatop)
ldatop
min(ldatop)
...
plot(score_comparison$topics, score_comparison$perplexity,
     type = 'b', lwd = 3, col = 1,
     las = 1,
...
plot(score_comparison$topics, score_comparison$coherence_score,
     type = 'b', lwd = 3, col = 2,
     las = 1,
     xlab = "Number of Topics",
     ylab = "Coherence Score",
     main = "Topic Coherence")
...
topic_terms <- topicmodels::posterior(k)$terms  # Rows = topics, Columns = terms
...
doc_topics <- topicmodels::posterior(lda9)$topics  # Matrix: documents x topics
...
topic_term_matrix <- topicmodels::posterior(k)$terms  # topics x terms
